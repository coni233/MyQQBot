...
输入 B 站 UID，通过 Danmakus 弹幕数据查询 TA 看过的直播主播：
...
- 普通模式：看过的主播列表（看过次数）
- `deep` 模式：完整成分分析——TA 的总弹幕 / 总消费 / 活跃天数 / 被拉黑次数，以及每位主播的「TA 弹幕数 / TA 消费 / 最近观看」+ 词云，分析约需十几秒
- `deep` 范围：`90`（最近 90 天）、`2024-01-01`（该日起至今）、`2024-01-01 2024-12-31`（指定区间）；不写为全量（2020-01-01 起）
- UID 支持 `UID:xxx` 格式输入
...
/查成分 deep 9200471
/查成分 deep 9200471 90
/查成分 deep 9200471 2024-01-01
```

数据来源：[ukamnads.icu](https://ukamnads.icu/swagger/index.html)（Danmakus，第三方非官方接口）。
