...
class BilibiliPrivateFollowingsError(Exception):
    """Raised when Bilibili hides a user's following list."""
...
async def get_user_info(uid: int) -> dict:
    # 旧接口 account.bilibili.com/api/member/getCardByMid 已失效。
    # 新接口的用户卡片位于 result["data"]["card"]。
    ...
    if result.get("code") != 0:
        raise RuntimeError(
            f"Bilibili user-info API failed: "
            f"{result.get('code')} {result.get('message')}"
        )

    data = result.get("data")
    if not isinstance(data, dict) or not isinstance(data.get("card"), dict):
        raise RuntimeError("Bilibili user-info API returned no card data")

    return data["card"]


async def get_attention_uids(uid: int) -> list[int]:
    """Return a user's followed-account UIDs from Bilibili's dedicated API."""
    url = "https://api.bilibili.com/x/relation/followings"
    attention_uids: list[int] = []
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        for page in range(1, 41):
            ...
            if result.get("code") != 0:
                if result.get("code") == 22115:
                    raise BilibiliPrivateFollowingsError
                raise RuntimeError(
                    "Bilibili followings API failed: "
                    f"{result.get('code')} {result.get('message')}"
                )

            data = result.get("data")
            if not isinstance(data, dict):
                raise RuntimeError("Bilibili followings API returned invalid data")

            followings = data.get("list")
            if followings is None:
                # Bilibili returns null when the follow list is private.
                return []
            if not isinstance(followings, list):
                raise RuntimeError("Bilibili followings API returned invalid list")

            if isinstance(data.get("total"), int):
                follows_total = data["total"]

            attention_uids.extend(int(following["mid"]) for following in followings)
            if not followings or (
                follows_total is not None and len(attention_uids) >= follows_total
            ):
                break
        else:
            logger.warning(f"Only fetched the first 2000 followings for UID {uid}")

    attention_uids = list(dict.fromkeys(attention_uids))
    if follows_total is not None and len(attention_uids) < follows_total:
        logger.warning(
            f"Only got {len(attention_uids)}/{follows_total} followings for UID {uid}"
        )
    else:
        logger.info(f"Got {len(attention_uids)} Bilibili followings for UID {uid}")
    return attention_uids
...
async def render_ddcheck_image(
    ...
    result = {
        ...
        "partial_followings": 0 < len(attentions) < follows_num,
        "followings_fetched": len(attentions),
        "vtbs": vtbs,
